...
import ScrapedContentModel, { ScrapedContentType } from '../schemas/data-capture/scraped-content.schema';
  ...
  try {
    ...
    for (const [url, { entryId, formId }] of Array.from(urlMap.entries())) {
      ...
      try {
        ...
        let scrapedContent = await ScrapedContentModel.findOneAndUpdate(
        ...
        scrapedContent = await ScrapedContentModel.findOneAndUpdate(
          { formId, url, scrapeDate },
          {
            title,
            description,
