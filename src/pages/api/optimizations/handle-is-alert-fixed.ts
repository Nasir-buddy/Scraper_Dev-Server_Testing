...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        try {
            const { userId, alertId, isAlertFixed } = req.body;
            console.log("userId", userId)
            console.log("alertId", alertId)
            console.log("isAlertFixed", isAlertFixed)
            if (isAlertFixed === undefined) {
                return res.status(400).json({ success: false, message: 'isAlertFixed is required' });
            }
            if (!userId || !alertId) {
            ...
            if (existingMessage) {
                ...
                existingMessage.isAlertFixed = isAlertFixed;
