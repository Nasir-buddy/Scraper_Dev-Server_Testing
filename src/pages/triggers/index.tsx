...
// function TriggersPage({ initialCheckedItems, quizId, idleSeconds }: TriggersPageProps) {
...
//   const [isAnyItemChecked, setIsAnyItemChecked] = useState<boolean>(initialCheckedItems.some(item => item.isChecked));
...
//     setIsLoading(true);
//     if (checkedItems.length > 0 && checkedItems[0].isChecked) {
//       checkedItems[0].triggersItemSlug = `${checkedItems[0].triggersItemSlug}_${extraInputValue}_seconds`;
//     }
//     try {
...
//       setIsLoading(false);
//     }
//   }, [checkedItems, extraInputValue, isModifying, quizId, router]);
...
//           <Box>
//             <Badge variant="surface" ml="4" radius="large" style={{ marginLeft: "40px" }}>
...
//           setExtraInputValue={setExtraInputValue}
//         />
...
//               <Button
//                 size="3"
//                 disabled={isLoading || !isAnyItemChecked || !isValid}
//                 loading={isLoading}
...
//     return {
//       props: {
//         initialCheckedItems: triggersFormData ? triggersFormData.triggersData : [],
...
function Trigger({
  ...
  console.log("Initial checked Item:",initialCheckedItems)
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [checkedItems, setCheckedItems] = useState<TriggersDataItem[]>(initialCheckedItems);
  const [extraInputValue, setExtraInputValue] = useState<string>(idleSeconds);
  const router = useRouter();
          ...
          >
            <Flex gap="10px" style={{ marginLeft: "auto" }}>
              {/* <Button
                ...
                disabled={isLoading}
                color="gray"
                onClick={() => router.push(isModifying ? '/basic' :`/questions?quizId=${quizId}`)}
                style={{ width: "150px", cursor: "pointer" }}
              >
                {isModifying ? "Cancel" : "Back"}
              </Button> */}
